...
export declare type MediaQueryItem = 'all' | 'print' | 'screen' | `(${'min-' | 'max-' | ''}${'aspect-ratio' | 'color' | 'color-gamut' | 'color-index' | 'display-mode' | 'grid' | 'inverted-colors' | 'height' | 'orientation' | 'pointer' | `prefers-${'color-scheme' | 'contrast' | 'reduced-motion' | 'reduced-transparency'}` | 'resolution' | 'scan' | 'width'}: ${string})`;
export declare type MediaQueryOperator = ' and ' | ' not ' | ' only ' | ', ';
export declare type MediaQueryString = MediaQueryItem | `${MediaQueryItem}${MediaQueryOperator}${MediaQueryItem}`;
