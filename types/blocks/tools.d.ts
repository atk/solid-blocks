...
export type MediaQueryItem = 'all' | 'print' | 'screen' | `(${'min-' | 'max-' | ''}${'aspect-ratio' | 'color' | 'color-gamut' | 'color-index' | 'display-mode' | 'grid' | 'inverted-colors' | 'height' | 'orientation' | 'pointer' | `prefers-${'color-scheme' | 'contrast' | 'reduced-motion' | 'reduced-transparency'}` | 'resolution' | 'scan' | 'width'}: ${string})`;
export type MediaQueryOperator = ' and ' | ' not ' | ' only ' | ', ';
export type MediaQueryString = MediaQueryItem | `${MediaQueryItem}${MediaQueryOperator}${MediaQueryItem}`;
...
export declare const runEvent: <E extends Event, T extends HTMLElement, EV = E & {
    currentTarget: T;
    target: Element;
}, EU = JSX.EventHandlerUnion<T, E>>(ev: EV, handler?: EU | undefined) => void;
