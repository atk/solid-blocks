...
const triggerHas = (trigger: TooltipProps['trigger'] | undefined, event: 'focus' | 'hover') =>
  trigger === undefined || trigger === event || Array.isArray(trigger) && trigger.includes(event);
...
export const Tooltip: Component<TooltipProps> = (props) => {
  ...
  createEffect(() => {
    ...
    setPositionStyle(
      local.position === 'nw'
      ? { top: `${wrapperRef.offsetTop}px`, left: `${wrapperRef.offsetLeft}px` }
      : local.position === 'n'
      ? { top: `${wrapperRef.offsetTop}px`, left: `${wrapperRef.offsetLeft  + (wrapperRef.offsetWidth >> 1)}px` }
      : local.position === 'ne'
      ? { top: `${wrapperRef.offsetTop}px`, left: `${wrapperRef.offsetLeft + wrapperRef.offsetWidth}px` }
      : local.position === 'e'
      ? {
      ...
      : local.position === 'sw'
      ? { top: `${wrapperRef.offsetTop + wrapperRef.offsetHeight}px`, left: `${wrapperRef.offsetLeft}px` }
      : local.position === 'w'
      ? { top: `${wrapperRef.offsetTop + (wrapperRef.offsetHeight >> 1)}px`, left: `${wrapperRef.offsetLeft}px` }
      : /* s is default */{
        top: `${wrapperRef.offsetTop + wrapperRef.offsetHeight}px`,
